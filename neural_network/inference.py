...
def inference(anoms_path, params_path, encoder_path, features, name, save_npy=False):

    filepaths = sorted(
        list(glob.glob(os.path.join(os.environ["CUBE_DIR"], "*_raw.nc")))
    )
    device = "cuda"
    ...
    means = (
        torch.tensor([MEANS[f] for f in features], dtype=float, device=device)
        .unsqueeze(1)
        .unsqueeze(1)
    )
    stds = (
        torch.tensor([STDS[f] for f in features], dtype=float, device=device)
        .unsqueeze(1)
        .unsqueeze(1)
    )
